...
<div className="flex flex-col gap-5">
  ...
  <div className="w-full flex justify-center px-4">
    <div className="w-full max-w-6xl flex items-center justify-between">
      <h1 className={cn("text-lg font-medium tracking-tight", darkTheme ? "text-white" : "text-black")}>
        Pattern vault
      </h1>
      <span className={cn(
        "text-xs px-2.5 py-1 rounded-full border transition-all",
        darkTheme ? "bg-white/5 border-white/10 text-white/40" : "bg-gray-100 border-black/8 text-gray-400",
      )}>
  ...
  <div className="w-full flex justify-center px-4">
    <div className="w-full max-w-6xl relative">
      <svg
        className={cn("absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none", darkTheme ? "text-white/30" : "text-gray-400")}
  ...
  <div className="w-full flex justify-center px-4">
    <div className="w-full max-w-6xl">
      <div className={cn(
        ...
        darkTheme ? "bg-white/5 border-white/8" : "bg-gray-100 border-black/6",
      )}>
        {["all","gradients","effects","decorative","geometric","animated","favorite"].map((cat) => (
          <button
            ...
            className={cn(
              ...
              category === cat
                ? cn("font-medium shadow-sm", darkTheme ? "bg-white text-black" : "bg-white text-gray-900")
                : cn(darkTheme ? "text-white/50 hover:text-white hover:bg-white/8" : "text-gray-500 hover:text-gray-800 hover:bg-white/80"),
            )}
          >
            {cat}
          </button>
  ...
  {category === "favorite" && finalPatterns.length === 0 && (
    <div className="w-full xl:p-10 flex justify-center px-4">
      <div className={cn(
        ...
        <div className={cn("w-10 h-10 rounded-full flex items-center justify-center", darkTheme ? "bg-white/5" : "bg-gray-100")}>
        ...
        <div className="flex flex-col items-center gap-1">
          <p className={cn("text-sm font-medium", darkTheme ? "text-white/60" : "text-gray-600")}>
            No favourites yet
          </p>
          <p className={cn("text-xs", darkTheme ? "text-white/25" : "text-gray-400")}>
  ...
  {category !== "favorite" && finalPatterns.length === 0 && (
    <div className="w-full xl:p-10 flex justify-center px-4">
      <div className={cn(
        ...
        <button
          onClick={() => { onClickSetCategory("all"); querySetOnInput(""); }}
          className={cn(
            "mt-1 text-xs px-3.5 py-1.5 rounded-lg border transition-all duration-150",
            darkTheme
              ? "border-white/10 text-white/40 hover:bg-white/5 hover:text-white/60"
