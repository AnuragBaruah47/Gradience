...
const GradientGenerator = () => {
  ...
  const createGradient = () => {
    if (type === "Linear") return linearGradientGenerate();
  ...
  const generate = () => {
    ...
    gsap.to(".gen-icon", {
      ...
      duration: 0.45,
      ease: "power2.inOut",
      onComplete: () => gsap.set(".gen-icon", { rotation: 0 }),
    });
    ...
    setTimeout(() => {
      ...
      setGradient(css);
      setLoading(false);
      setHistory((prev) => [css, ...prev].slice(0, 16));
      setCopied(false);
  ...
  return (
    <div className="flex flex-col gap-5 p-4 sm:p-6 w-full max-w-2xl mx-auto">
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-0.5">
        {TYPES.map(({ label, icon }) => (
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 px-4 py-2 rounded-[9px] text-[13px] border transition-all duration-150 shrink-0",
              type === label
                ? darkTheme
      ...
      <div
        ...
        className={cn(
          "relative w-full h-48 sm:h-56 rounded-2xl border overflow-hidden transition-[background] duration-500",
      ...
      <div
        ...
        className={cn(
          "w-full rounded-xl text-center px-4 py-3.5 text-[12px] leading-relaxed break-all border font-mono",
          darkTheme
